...
##### Krzychs
response = requests.get('https://www.olx.pl/nieruchomosci/mieszkania/wynajem/warszawa/?search%5Bfilter_enum_furniture%5D%5B0%5D=yes&search%5Bfilter_float_m%3Afrom%5D=35')

html = response.content
soup = BeautifulSoup(html, 'lxml')


#xpath = find_by_xpath(soup, '//*[@id="offers_table"]/tbody')
table_first = soup.find_all('table',attrs={'class':'fixed offers breakword redesigned'})
table_second = soup.find_all('table',attrs={'class':'fixed offers breakword offers--top redesigned'})
tables = [table_first[0].find_all('a', href=True), table_second[0].find_all('a', href=True)]
#table_elements = table[0].find_all('a', href=True)


flats = []
for table_elements in tables:
    for row, content in enumerate(table_elements):
        flat = content['href']
        if flat != '#':
...
for a in flats:
    ...
    if True:
        ...
        row = {"Indeks": indeks, "Cena całkowita": cena, "Metraż": metraz, "Link": a}
